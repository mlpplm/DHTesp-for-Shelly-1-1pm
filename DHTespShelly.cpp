/* Shelly version of DHT library. The Rx/Tx pin has been split into 
 Rx and Tx pins due to Shelly 1/1PM Temperature addon. The reason is 
 that this addon uses directional opto-isolators in order to isolate 
 the high voltage and small signal sections. 

 USAGE: dht.setup(<dht22_pinin>, <dht22_pinout>, DHTesp::<DHT22>); instead ht.setup(<dht22_pin>, DHTesp::<DHT22>);

 Just add input and output pins (Shelly using GPIO03 for input and 
 GPIO00 for output for DHT).
 
 Tested only with DHT22, but the other DHTs should work, as well. (Remember 
 adding a 4.7k pull-up resistor between signal and Vcc)

 NEVER, EVER, connect anything directly to Shelly 1/1PM's GPIOs. The GPIOs are directly connected to
 230V mains. Instead, use the official temperature addon.
...
#include "DHTespShelly.h"

void DHTesp::setup(uint8_t pinin, uint8_t pinout, DHT_MODEL_t model)
{
	DHTesp::pinin = pinin;
  DHTesp::pinout = pinout;
	DHTesp::model = model;
...
void DHTesp::readSensor()
{
	...
	// Request sample
	digitalWrite(pinout, LOW); // Send start signal
	...
	pinMode(pinin, INPUT);
	digitalWrite(pinout, HIGH); // Switch bus to receive data
	...
	for (int8_t i = -3; i < 2 * 40; i++)
	{
		...
		} while (digitalRead(pinin) == (i & 1) ? HIGH : LOW);
